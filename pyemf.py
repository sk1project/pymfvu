...
## Added by frob
    class _GDICOMMENT(_EMR_UNKNOWN):
        """Comment can contain a lot of significant information."""
        emr_id=70
        emr_typedef=[
                ('i','DataSize'),
                ('i','Ident'), ## for public comments has to be always GDICOMMENT_IDENTIFIER
                ('i','iComment') ## encode type of comment
                ]

        def __init__(self):
            _EMR_UNKNOWN.__init__(self)
##            if iComment == GDICOMMENT_WINDOWS_METAFILE:
##                ('i','nVersion'),
##                ('i','nChecksum'),
##                ('i','fFlags'),
##                ('i','cbWinMetaFile') ## size of embedded WMF

##                if iComment == GDICOMMENT_BEGINGROUP:
##                    ('i','rclBox_left'),
##                    ('i','rclBox_top'),
##                    ('i','rclBox_right'),
##                    ('i','rclBox_bottom'),
##                    ('i','nDescription') ## Num of chars in optional Unicode description
##                if iComment == GDICOMMENT_ENDGROUP:
##                    pass                ## there is no more fields for EndGroup, just 'Ident' and 'iComment'
                
##                if iComment == GDICOMMENT_MULTIFORMATS:
##                    ('i','rclBox_left'),
##                    ('i','rclBox_top'),
##                    ('i','rclBox_right'),
##                    ('i','rclBox_bottom'),
##                    ('i','nFormats'),  ## numb of formats in the comment
##                    (EMRFORMAT aemrformat[1]) ## array of EMRFORMAT structures
