#!/usr/bin/env python
# pyvmfvu -- Utility to view and research WMF (Windows Metafile) files.
...
__description__ = "Python Windows Metafile viewer"
...
##        action_group.add_toggle_actions([( "NewToggle", gtk.STOCK_INDEX,                    # name, stock id
##            "Ne_wToggle", "<control>W",                     # label, accelerator
##            "Toggle some window",                                    # tooltip
##            self.activate_records, False)])
        return action_group
    ...
    def activate_records(self, action):
        ...
        if pn != -1:
            ...
            for i in range(len(records)):
                string = string+'%u  %04x   '%(i,offset)+str(records[i])
    ...
    def activate_open(self,parent=None):
        ...
        if fname:
            try:
                ...
                page.height = 768
                page.scale = 1
                page.file.load(fname)
                wmfcmd.parse(page)
                dnum = len(self.das)
                self.das[dnum] = wmfdoc.Face(page)
                ...
                self.das[dnum].page.hadj = gtk.Adjustment(0.0, 0.0,  len(self.das[dnum].page.cmds)+1, 0.1, 1.0, 1.0)
    ...
    def activate_save(self,parent=None):
        ...
        if fname:
            ...
            cr.save()
            wmfdraw.render(self.das[pn],cr,self.das[pn].page)
            cr.restore()
            cr.show_page()
            surface.flush()
            surface.finish()
